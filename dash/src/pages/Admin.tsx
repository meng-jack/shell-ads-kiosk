...
function ActiveRow({ ad, index, total, onMoveUp, onMoveDown, onDelete }: ActiveRowProps) {
  return (
    <div className="adm-row">
      ...
      <div className="adm-row-info">
        ...
        <span className="adm-row-meta">
          ...
          <span className="adm-row-dur">{(ad.durationMs / 1000).toFixed(0)}s</span>
          {ad.src && (
            <a className="adm-row-url" href={ad.src} target="_blank" rel="noreferrer" title={ad.src}>
              {truncate(ad.src, 40)}
            </a>
          )}
        </span>
      ...
      <div className="adm-row-actions">
        <button className="adm-icon-btn" onClick={onMoveUp} disabled={index === 0} title="Move up">↑</button>
        <button className="adm-icon-btn" onClick={onMoveDown} disabled={index === total - 1} title="Move down">↓</button>
        <button className="adm-icon-btn adm-icon-btn--del" onClick={onDelete} title="Delete">✕</button>
      </div>
    </div>
  )
}
...
function PendingRow({ ad, onApprove, onDelete }: PendingRowProps) {
  return (
    <div className="adm-row">
      ...
      <div className="adm-row-actions">
        <button className="adm-icon-btn adm-icon-btn--approve" onClick={onApprove} title="Approve → active">✓</button>
        <button className="adm-icon-btn adm-icon-btn--del" onClick={onDelete} title="Delete">✕</button>
      </div>
    </div>
  )
    ...
    <div className="adm-wrap">
      ...
      <div className="adm-header">
        ...
        <div className="adm-header-actions">
          <button className="adm-btn" onClick={reload}>Reload kiosk</button>
          <button className="adm-btn adm-btn--ghost" onClick={logout}>Sign out</button>
        </div>
      </div>

      {err && <div className="adm-err-banner">{err} <button onClick={fetchState}>Retry</button></div>}
      ...
      <section className="adm-section">
        <div className="adm-section-header">
          ...
          {active.length > 0 && (
            <button className="adm-btn adm-btn--ghost adm-btn--sm" onClick={clearAll}>
              Clear all
            </button>
          )}
        </div>
        ...
        {active.length === 0
          ? <p className="adm-empty">No active ads. Approve pending items or submit via the main page.</p>
          : (
      ...
      <section className="adm-section">
        <div className="adm-section-header">
          ...
          {pending.length > 0 && (
            <button className="adm-btn adm-btn--ghost adm-btn--sm" onClick={approveAll}>
              Approve all
            </button>
          )}
        </div>
